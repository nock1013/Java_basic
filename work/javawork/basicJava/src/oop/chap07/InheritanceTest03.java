...
/*
 * <상속관계에서 생성자가 갖는 특징>
 * 1. 클래스의 모든 생성자 메서드의 첫 번째 문장에는 
 *    부모 클래스의 기본생성자를 호출하는 명령문이 생략되어 있다.
 ...
 * 3. 부모 클래스에 정의되어 있는 멤버변수가 값을 세팅해야 하는 경우에도
...
class SubA extends SuperA {
	...
	int point;
	SubA() {
		super();			//모든 생성자 메서드의 첫 번째 문장에는 super()호출
	}
	SubA(String addr) {
		super();
		this.addr = addr;
	...
	SubA(String addr, int point) {
		this(addr);			// 현재 객체에 이미 정의되어 있는 나의 또 다른 생성자를 호출
		this.point = point;
	}
